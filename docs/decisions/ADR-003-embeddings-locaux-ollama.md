...
- **Statut** : Accepté (2026-09-19) — pour les **embeddings**. La partie **génération** de cet ADR est **SUPERSÉDÉE** par **ADR-004** (OpenCode Go).
- **Décideur** : Samen Steeve
- **Remplace partiellement** : ADR-002 (pour la partie embeddings)
- **Remplacé partiellement par** : ADR-004 (pour la partie génération)
...
- **Génération** : ~~OpenRouter (`openai/gpt-4.1-mini`), via la credential OpenRouter~~ → **superseded par ADR-004** (OpenCode Go `deepseek-v4-flash-vision-exp`).

## Justification

- **Aucun coût d'API** pour l'indexation ; pas besoin de clé OpenAI.
- **Confidentialité renforcée** : les notes ne quittent pas le VPS pour l'embedding (seuls les chunks récupérés + la question partent vers **OpenCode Go** pour la génération — voir ADR-004).
...
- ~~OpenRouter reste nécessaire pour la génération~~ → la génération passe par **OpenCode Go** (ADR-004).
