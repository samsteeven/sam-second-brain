...
- **Ingestion** : **IDs de points déterministes** (hash `fichier + contenu du chunk`) + **embeddings différentiels** (seuls les chunks nouveaux/modifiés sont recalculés) + **nettoyage des orphelins horodaté** (`indexed_at < runStart`) → **idempotente, rapide (~2 s) et sans perte**, même en concurrence. Ne jamais revenir au schéma « vider puis réinsérer ».
