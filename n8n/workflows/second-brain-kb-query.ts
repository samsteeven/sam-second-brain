...
const qdrantSearch = vectorStore({
  ...
  output: [{ document: { pageContent: '## Mon projet\nContenu...', metadata: { file: '03-Projects/MonProjet.md' } } }],
});
...
const httpChat = node({
  ...
  output: [{ choices: [{ message: { content: "D'après tes notes..." } }] }],
});
  ...
  config: {
    ...
    parameters: {
      ...
      jsCode: 'const answer = $json.choices?.[0]?.message?.content ?? "Erreur : aucune réponse du modèle.";\n' +
        ...
        'return { json: { response: { answer, sources } } };',
    },
  },
  output: [{ response: { answer: 'Réponse...', sources: ['03-Projects/EasyPharma.md'] } }],
});

export default workflow('sam-second-brain-kb-query', 'Second Brain — KB Query')
  .add(subTrigger)
  .to(normalize)
  .to(qdrantSearch)
  .to(buildContext)
