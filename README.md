...
- **Ingestion automatique** : les notes Markdown modifiées sont chunkées, embarquées et indexées dans Qdrant (schedule 30 min, notes `pending` exclues). IDs de points **déterministes** → ingestion **idempotente** (aucun doublon, même en cas d'exécutions concurrentes) et **embeddings différentiels** (seuls les chunks nouveaux/modifiés sont recalculés → ~2 s en régime stable).
